...
class RandomJunk {
	...
	public static function run() {
		...
		for($i = 0; $i < self::$count_dir; $i++) {
			$rand = mt_rand(0,2);
			if($rand === 0) { $curpath = $curpath."/".self::randomName(); }
			else if($rand === 1 && $curpath != realpath(dirname(__FILE__))."/junk") { $curpath = substr($curpath, 0, strrpos($curpath, "/")); }
	...
	public static function randomName($length = null) {
		if( $length === null ) { $length = mt_rand(10,20); }
